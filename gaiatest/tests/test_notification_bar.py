...
class TestNotificationBar(GaiaTestCase):
    ...
    def test_notification_bar(self):
        ...
        self.wait_for_condition(lambda m: self.marionette.find_element(*self._notification_toaster_locator).location['y'] == 0)

        # TODO Re-enable this when Bug 861874
        # self.wait_for_element_not_displayed(*self._notification_toaster_locator)
        self.wait_for_condition(lambda m: self.marionette.find_element(*self._notification_toaster_locator).location['y'] == -50)
